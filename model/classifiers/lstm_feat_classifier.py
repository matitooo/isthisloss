...
class LSTMFeaturesClassifier(nn.Module):
    def __init__(self, embedded_stack_size, lstm_hidden_size, lstm_n_layers,n_features):
        ...
        """
        super(LSTMFeaturesClassifier, self).__init__()

        self.lstm = LSTM(embedded_stack_size, lstm_hidden_size, lstm_n_layers)
        dense_layer_size=int(lstm_hidden_size/2)+n_features

        # Final classifier: only uses the context vector from LSTM (no concatenation)
        self.classifier = nn.Linear(dense_layer_size,2)
    ...
    def forward(self, embedded_stack,extracted_features, lstm_states=None):
        ...
        output= self.lstm(embedded_stack, lstm_states)  # context: (B, lstm_hidden_size)
        # Concatenate attention output and manually extracted features
        concatenation=torch.cat((output,extracted_features),dim=-1)
        # Final classification layer uses only the concatenated vector
